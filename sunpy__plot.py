...
from . import sunpy__synthetic_image  # used for images with noise, pixel scaling, etc.
import matplotlib
...
def plot_spectrum(
    filelist, savefile="spectrum.pdf", fontsize=14, ymin=None, ymax=None, **kwargs
):
    ...
    fig.subplots_adjust(
        left=0.18, right=0.95, top=0.95, bottom=0.12, wspace=0.0, hspace=0.0
    )
...
def plot_synthetic_hst(
    filename, savefile="syn_hst.png", **kwargs
    ...
    rp, img = return_synthetic_hst_img(
        filename, **kwargs
...
def return_synthetic_sdss_gri_img(
    filename,
    lupton_alpha=0.5,
    lupton_Q=0.5,
    scale_min=1e-4,
    ...
    while fail_flag and (n_iter < 2):
        ...
        try:
            seed = (
                int(
                    filename[
                        filename.index("broadband_") + 10 : filename.index(".fits")
                    ]
                )
                * (n_iter)
                * seed_boost
            ...
            try:
                seed = (
                    int(filename[filename.index(".fits") - 3 : filename.index(".fits")])
                    * (n_iter)
                    * seed_boost
                )
            except:
        ...
        (
            ...
            the_used_seed,
            this_fail_flag,
        ) = sunpy__synthetic_image.build_synthetic_image(
            filename,
            "g_SDSS.res",
            seed=seed,
...
def return_synthetic_hst_img(
    ...
    **kwargs
    ...
    try:
        seed = int(
            filename[filename.index("broadband_") + 10 : filename.index(".fits")]
        )
    except:
        try:
            seed = int(
                filename[
                    filename.index("broadband_red_") + 14 : filename.index(".fits")
                ]
            )
        except:
            seed = int(
                filename[
                    filename.index("broadband_rest_") + 15 : filename.index(".fits")
                ]
            )
    ...
    img[img < 0] = 0
    print("img min/max/mean " + str(img.min()) + "  " + str(img.max()) + "  " + str(
        img.mean()
    ))
    print(" ")
...
def return_sdss_gri_img(
    filename, camera=0, scale_min=0.1, scale_max=50, size_scale=1.0, non_linear=0.5
):
    ...
    b_image = (
        sunpy__load.load_broadband_image(filename, band="g_SDSS.res", camera=camera)
        * 0.7
    )
    g_image = (
        sunpy__load.load_broadband_image(filename, band="r_SDSS.res", camera=camera)
        * 1.0
    )
    r_image = (
        sunpy__load.load_broadband_image(filename, band="i_SDSS.res", camera=camera)
        * 1.4
    )
    n_pixels = r_image.shape[0]
    img = np.zeros((n_pixels, n_pixels, 3), dtype=float)
    ...
    img[:, :, 0] = asinh(
        r_image, scale_min=scale_min, scale_max=scale_max, non_linear=non_linear
    )
    img[:, :, 1] = asinh(
        g_image, scale_min=scale_min, scale_max=scale_max, non_linear=non_linear
    )
    img[:, :, 2] = asinh(
        b_image, scale_min=scale_min, scale_max=scale_max, non_linear=non_linear
    )
    img[img < 0] = 0
...
def return_johnson_uvk_img(
    filename, camera=0, scale_min=0.1, scale_max=50, size_scale=1.0
):
    if not os.path.exists(filename):
        print("file not found:", filename)
        sys.exit()
    ...
    b_image = (
        sunpy__load.load_broadband_image(filename, band="U_Johnson.res", camera=camera)
        * b_effective_wavelength
        / g_effective_wavelength
        * 2.5
    )
    g_image = (
        sunpy__load.load_broadband_image(filename, band="V_Johnson.res", camera=camera)
        * g_effective_wavelength
        / g_effective_wavelength
    )
    r_image = (
        sunpy__load.load_broadband_image(filename, band="K_Johnson.res", camera=camera)
    ...
    img = np.zeros((n_pixels, n_pixels, 3), dtype=float)
    img[:, :, 0] = asinh(
        r_image, scale_min=scale_min, scale_max=scale_max, non_linear=0.5
    )
    img[:, :, 1] = asinh(
        g_image, scale_min=scale_min, scale_max=scale_max, non_linear=0.5
    )
    img[:, :, 2] = asinh(
        b_image, scale_min=scale_min, scale_max=scale_max, non_linear=0.5
    )
    img[img < 0] = 0
    return img


def return_stellar_mass_img(
    filename, camera=0, scale_min=1e8, scale_max=1e10, size_scale=1.0, non_linear=1e8
):
    image = sunpy__load.load_stellar_mass_map(filename, camera=camera)
    n_pixels = image.shape[0]
    img = np.zeros((n_pixels, n_pixels), dtype=float)
    img[:, :] = asinh(
        image, scale_min=scale_min, scale_max=scale_max, non_linear=non_linear
    )
    img[img < 0] = 0
    return img


def return_mass_weighted_age_img(
    filename, camera=0, scale_min=None, scale_max=None, size_scale=1.0
):
...
def return_stellar_metal_img(
    filename, camera=0, scale_min=None, scale_max=None, size_scale=1.0, non_linear=None
):
...
def my_save_image(
    ...
    top_opt_text=None,
    full_fov=None,
    cut_bad_pixels=False,
    zoom=None,
    save_indiv_bands=True,
    **kwargs
):
    if img.shape[0] > 1:
        ...
        if zoom != None:
            ...
            print("resized image from " + str(n_pixels_current) + " to " + str(
                img.shape[0]
            ) + " pixels")
            if full_fov != None:
                full_fov /= 1.0 * zoom
        ...
        if not full_fov == None:
            ...
            ax.plot(
                [center - bar_size_in_pixels / 2.0, center + bar_size_in_pixels / 2.0],
                [0.1 * img.shape[0], 0.1 * img.shape[0]],
                lw=2,
                color="w",
            )
