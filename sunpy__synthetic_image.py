...
def build_synthetic_image(filename, band, r_petro_kpc=None, openlist=None, **kwargs):
    ...
    obj = synthetic_image(
        filename, band=band, r_petro_kpc=r_petro_kpc, openlist=openlist, **kwargs
    )
...
def load_resolved_broadband_apparent_magnitudes(
    filename, redshift, camera=0, seed=12345, n_bands=36, **kwargs
):
    ...
    for band in np.arange(n_bands):
        obj = synthetic_image(
            filename, band=int(band), seed=seed, redshift=redshift, **kwargs
        )
    ...
    for band in np.arange(n_bands):
        ...
        if verbose:
            print("the ab magnitude of band " + str(band) + " is :" + str(
                abmag
            ) + "  " + str(mags[band]))
    ...
    dist = (cosmocalc.cosmocalc(redshift, H0=70.4, WM=0.2726, WV=0.7274))[
        "DL_Mpc"
    ] * 1e6
...
class synthetic_image:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.image_header = hdulist[
            "CAMERA" + str(camera) + "-BROADBAND-NONSCATTER"
        ].header
        ...
        self.sunrise_absolute_mag = (
            self.filter_data["AB_mag_nonscatter" + str(self.camera)]
        )[band]
        hdulist.close()
        ...
        self.rp_image = (
            single_image()
        )  # scale image based on rp radius criteria (for GZ)
        self.bg_image = (
            single_image()
        )  # add backgrounds (only possible for 5 SDSS bands at the moment)
        # ============ SET ORIGINAL IMAGE ======================#
        all_images, self.openlist = sunpy.sunpy__load.load_all_broadband_images(
        ...
        if verbose:
            print("preparing to save " + filename[
                : filename.index(bb_label)
            ] + "synthetic_image_" + filename[
                filename.index(bb_label) + num_label : filename.index(".fits")
            ] + "_band_" + str(
                self.band
            ) + "_camera_" + str(
                camera
    ...
    def convolve_with_psf(self, add_psf=True):
        if add_psf:
            if self.telescope.psf_fits_file != None:
                ...
                n_pixel_new = (
                    self.sunrise_image.n_pixels
                    * self.sunrise_image.pixel_in_arcsec
                    / self.telescope.psf_pixsize_arcsec
                )

                # print np.sum(self.sunrise_image.image)
                new_image = congrid(
                    self.sunrise_image.image, (n_pixel_new, n_pixel_new)
                )
                ...
                if CONVOLVE_TYPE == "astropy":
                    ...
                    conv_im = convolve(
                        new_image,
                        self.telescope.psf_kernel / np.sum(self.telescope.psf_kernel),
                        boundary="fill",
                        fillvalue=0.0,
                        mode="same",
                    )  # boundary option?
    ...
    def add_gaussian_psf(
        self, add_psf=True, sample_factor=1.0
    ):  # operates on sunrise_image -> creates psf_image
        if add_psf:
            current_psf_sigma_pixels = (
                self.telescope.psf_fwhm_arcsec
                * (1.0 / 2.355)
                / self.sunrise_image.pixel_in_arcsec
            )

            if (
                current_psf_sigma_pixels < 8
                ...
                if (
                    n_pixel_new > 1500
                ):  # for speed, beyond this, the PSF is already very small...
                    n_pixel_new = 1500

                target_psf_sigma_pixels = (
                    n_pixel_new * current_psf_sigma_pixels / self.sunrise_image.n_pixels
                )
            ...
            dummy = sp.ndimage.filters.gaussian_filter(
                new_image, current_psf_sigma_pixels, output=psf_image, mode="constant"
            )
    ...
    def rebin_to_physical_scale(self, rebin_phys=True):
        if rebin_phys:
            n_pixel_new = np.floor(
                (self.psf_image.pixel_in_arcsec / self.telescope.pixelsize_arcsec)
                * self.psf_image.n_pixels
            )
    ...
    def add_noise(self, add_noise=True, sky_sig=None, sn_limit=25.0):
        if add_noise:
            ...
            noise_image = sky_sig * np.random.randn(
                self.rebinned_image.n_pixels, self.rebinned_image.n_pixels
            )
    ...
    def calc_r_petro(self, r_petro_kpc=None, resize_rp=True):  # rename to "set_r_petro"
        ...
        if resize_rp == False:
            ...
            r_petro_kpc = (
                RadiusObject.PetroRadius * self.noisy_image.pixel_in_kpc
    ...
    def resize_image_from_rp(self, resize_rp=True, resize_factor=0.016, max_rp=100.0):
        if resize_rp:
            if self.r_petro_kpc < max_rp:
                rp_pixel_in_kpc = (
                    resize_factor * self.r_petro_kpc
            ...
            Ntotal_new = int(
                (self.noisy_image.pixel_in_kpc / rp_pixel_in_kpc)
            ...
            self.rp_image.init_image(
                rp_image,
                self,
                fov=(1.0 * n_pixels_galaxy_zoo) * (resize_factor * self.r_petro_kpc),
            )
            ...
            self.rp_image.init_image(
                ...
                self,
                fov=self.noisy_image.pixel_in_kpc * self.noisy_image.n_pixels,
            )
    ...
    def add_background(
        ...
        if add_background and (len(backgrounds[self.band]) > 0):
            bg_image = (
                10.0 * self.rp_image.image
            )  # dummy values for while loop condition
            ...
            while tot_bg > tol_fac * tot_img:
                ...
                Npix_get = np.floor(
                    self.rp_image.n_pixels * self.rp_image.pixel_in_arcsec / pixsize
                )
                ...
                bg_image_raw = im[
                    starti : starti + Npix_get, startj : startj + Npix_get
                ]  # the extracted patch...
                # === need to convert to microJy / str ===#
                bg_image_muJy = bg_image_raw * 10.0 ** (
                    -0.4 * (bg_zpt[self.band_name][0] - 23.9)
        ...
        self.bg_image.init_image(
            new_image, self, fov=self.rp_image.pixel_in_kpc * self.rp_image.n_pixels
        )
        del new_image
        gc.collect()
        return seed
    ...
    def save_bgimage_fits_mujyas(
        self,
        outputfitsfile,
        save_img_in_muJy=False,
        add_noise=False,
        ...
        AB_zeropoint = (
            -2.5 * np.log10(theobj.pixel_in_arcsec ** 2)
            - 2.5 * (-6.0)
            + 2.5 * np.log10(3631.0)
        )  # for image in muJy/Arcsec**2
        total_apparent_mag = -2.5 * np.log10(np.sum(image)) + AB_zeropoint
        total_absolute_mag = (
            -2.5 * np.log10(np.sum(image))
            + AB_zeropoint
        ...
        sun_AB_app_zp = (
            -2.5 * np.log10(sunobj.pixel_in_arcsec ** 2)
            - 2.5 * (-6.0)
            + 2.5 * np.log10(3631.0)
        )
        sun_AB_cam_zp = (
            -2.5 * np.log10(sunobj.camera_pixel_in_arcsec ** 2)
            - 2.5 * (-6.0)
            + 2.5 * np.log10(3631.0)
        )
        ...
        primhdu.header.update(
            "IMUNIT", "muJy/SqArcsec", comment="microjanskies per square arcsecond"
        )
        primhdu.header.update("ABZP", round(AB_zeropoint, 6), "For Final Image")
        primhdu.header.update(
            "PIXSCALE", round(theobj.pixel_in_arcsec, 6), "For Final Image, arcsec"
        )
        primhdu.header.update(
            "PIXORIG",
            round(theobj.camera_pixel_in_arcsec, 6),
            "For Original Image, arcsec",
        )
        primhdu.header.update("PIXKPC", round(theobj.pixel_in_kpc, 6), "KPC")
        primhdu.header.update(
            "ORIGKPC",
            round(self.sunrise_image.pixel_in_kpc, 6),
            "For Original Image, KPC",
        )
        primhdu.header.update("NPIX", theobj.n_pixels)
        primhdu.header.update("NPIXORIG", self.sunrise_image.n_pixels)
        ...
        primhdu.header.update(
            "DISTMOD", round(self.cosmology.distance_modulus, 6), "Mag"
        )
        ...
        if self.telescope.psf_fits_file == None:
            primhdu.header.update(
                "PSFFWHM", round(self.telescope.psf_fwhm_arcsec, 6), "arcsec"
            )
        else:
            primhdu.header.update(
                "PSFFILE",
                os.path.join(
                    os.path.basename(os.path.dirname(self.telescope.psf_fits_file)),
                    os.path.basename(self.telescope.psf_fits_file),
                ),
            )

        primhdu.header.update(
            "TPIX", round(self.telescope.pixelsize_arcsec, 6), "arcsec"
        )
        ...
        primhdu.header.update(
            "SUNMAG",
            round(sunrise_absolute_mag, 6),
            "from spectrum, Note: excludes Lyman absorption",
        )
        primhdu.header.update(
            "SUNCMAG", round(sunrise_image_camera_mag, 6), "from image, camera mag"
        )
        primhdu.header.update(
            "SUNAPMAG", round(sunrise_image_apparent_mag, 6), "from image, apparent mag"
        )
        primhdu.header.update(
            "SUABSMAG", round(sunrise_image_absolute_mag, 6), "from image, absolute mag"
        )
...
class RadialInfo:
    ...
    def __init__(self, N, image, num_pts=100, max_pixels_for_fit=100000):
        ...
        xgrid = np.linspace(
            float(-self.Npix) / 2.0 + 0.5, float(self.Npix) / 2.0 - 0.5, num=self.Npix
        )
        ...
        for index, val in enumerate(y1[:-1]):
            if index == 0:
                ...
                y1sum[index] = (
                    y1sum[index - 1]
                    + 3.14159
                    * (self.RadiusGrid[index + 1] ** 2 - self.RadiusGrid[index] ** 2)
                    * (y1[index] + y1[index + 1])
                    / 2.0
                )
                y2sum[index] = (
...
class cosmology:
    def __init__(self, redshift, H0=70.4, WM=0.2726, WV=0.7274):
        ...
        self.lum_dist = (
            cosmocalc.cosmocalc(self.redshift, H0=self.H0, WM=self.WM, WV=self.WV)
        )[
            "DL_Mpc"
    ...
    def __init__(
        ...
        if psf_fits != None:
            ...
            if psf_truncate_pixels != None:
                ...
                orig_psf_kernel = orig_psf_kernel[
                    psfc - st : psfc + st, psfc - st : psfc + st
                ]
            ...
            if (
                (self.psf_pixsize_arcsec > self.pixelsize_arcsec)
                and (rebin_phys == True)
                and (add_psf == True)
            ):
...
class single_image:
    ...
    def init_image(self, image, parent_obj, fov=None, comoving_to_phys_fov=False):
        ...
        if fov == None:
            if comoving_to_phys_fov:
                self.pixel_in_kpc = (
                    parent_obj.param_header.get("linear_fov")
                    / self.n_pixels
                    / (parent_obj.cosmology.redshift + 1)
                )
            else:
                self.pixel_in_kpc = (
                    parent_obj.param_header.get("linear_fov") / self.n_pixels
                )
        else:
        ...
        self.camera_pixel_in_arcsec = (
            self.pixel_in_kpc / parent_obj.param_header.get("cameradist")
        ) * 2.06e5
    ...
    def calc_ab_abs_zero(self, parent_obj):
        ...
        to_microjanskies_at_10pc = (
            to_microjanskies * (cameradist_in_kpc / abs_dist) ** 2
        )
    ...
    def convert_orig_to_nanomaggies(self, parent_obj):
        ...
        orig_to_nmaggies = distance_factor * 10.0 ** (
            0.4 * (22.5 - self.ab_abs_zeropoint)
        )
